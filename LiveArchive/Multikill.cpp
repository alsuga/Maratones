...
pair<point, point> compute_center(point a, point b) {
  double d = dist(a,b) / 2.0;
  double h = sqrt(r*r - d*d);
  point vec = (a - b);
  vec = vec / norm(vec);
  D(h);
  point op1 = (RotateCW90(vec) * h ) + (b + ((a -b) * 0.5));
  point op2 = (RotateCCW90(vec) * h) + (b + ((a -b) * 0.5));
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i + 1; j < n; ++j) {
      if (cmp(dist(p[i], p[j]), dia) == -1) {
        ...
        D(p[j]);
        D(center.first);
        D(dist(center.first, p[i]));
        D(center.second);
        ans = max(ans, count(center.first));
        D(count(center.first));
        ans = max(ans, count(center.second));
        D(count(center.second));
        cout<<"------------------------------------"<<endl;
